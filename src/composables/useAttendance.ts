...
export function useAttendance(props: { id: number; initialName: string }) {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      const response = await apiClient.get<AttendanceApiResponse>(
        `${API_BASE_URL}/attendance/records?userId=${userId.value}&targetMonth=${targetMonth.value}`,
      )
    ...
    return daysInMonth.map((dateStr) => {
      ...
      if (record) {
        ...
        record.breaks.forEach((b) => {
          totalBreakMs +=
            new Date(b.breakEndDt).getTime() -
            new Date(b.breakStartDt).getTime()
        })
        ...
        if (record.workStartDt && record.workEndDt) {
          const diffMs =
            new Date(record.workEndDt).getTime() -
            new Date(record.workStartDt).getTime() -
            totalBreakMs
        ...
        return {
          ...
          day: new Date(dateStr).toLocaleDateString('ja-JP', {
            weekday: 'short',
          }),
  ...
  const monthlyData = computed<AttendanceRow[]>(() => {
    ...
    // 勤務日数
    const totalWorkingDays = dailyData.value.filter(
      (day) => day.startTime !== '-',
    ).length
